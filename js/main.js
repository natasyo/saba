$(document).ready(function() {
    ...
    $('.team__peoples').slick({
        prevArrow: '<button type="button" class="slick-prev"><img src="images/red-left.png" alt=""><span class="team__people-other team__people-prev"></span></button>',
        nextArrow: '<button type="button" class="slick-next"><span class="team__people-other team__people-next"></span><img src="images/red-right.png" alt=""></button>',
        autoplay: true,
        autoplaySpeed: 5000,
        dots: true,
        responsive: [{
    ...
    $('.parnter__partners').slick({
        prevArrow: '<button type="button" class="slick-prev"><img src="images/red-left.png" alt=""></button>',
        nextArrow: '<button type="button" class="slick-next"><img src="images/red-right.png" alt=""></button>',
        autoplay: true,
        autoplaySpeed: 3000,
        slidesToShow: 4,
        slidesToScroll: 4,
    ...
    if ($(window).width() > 992) {
        ...
        var step = [];
        var current = [];
        $.each($('.about-us-section3__counter-number'), function(indexInArray, valueOfElement) {
            ...
            if (flag[indexInArray]) {
                $(window).scroll(function() {
                    if ($(object).offset().top < $(window).scrollTop() + $(window).height() && $(object).data('value') >= $(object).text() && $(window).width() > 991) {

                        flag[indexInArray] = false;
                        var count = String($(object).data('value'));
                        interval[indexInArray] = setInterval(() => {
                            current[indexInArray] = (current[indexInArray] + step[indexInArray] > count) ? count : current[indexInArray] + step[indexInArray];
                            var currentStr = String(current[indexInArray]);
                            currentStr = currentStr.replace(new RegExp("^(\\d{" + (currentStr.length % 3 ? currentStr.length % 3 : 0) + "})(\\d{3})", "g"), "$1 $2").replace(/(\d{3})+?/gi, "$1 ").trim();
    ...
    if ($(window).width() <= 992) {
        ...
        $('.projects__project-overlay-open').click(function(e) {
            e.stopPropagation();
            e.preventDefault();
            if ($(this).parents('.projects__project-overlay').hasClass('projects__project-overlay_show')) {
                $('.projects__project-overlay').removeClass('projects__project-overlay_show');
            } else {

                $('.projects__project-overlay').removeClass('projects__project-overlay_show');
                $(this).parents('.projects__project-overlay').addClass('projects__project-overlay_show');
            }
    ...
    $(document).mouseup(function(e) { // событие клика по веб-документу
        ...
        if (!div.is(e.target) // если клик был не по нашему блоку
            &&
            div.has(e.target).length === 0 && $('.header__main-menu')) { // и не по его дочерним элементам
...
function setMap() {
    ...
    function init() {
        ...
        $('.contats__departament').each(function(index, val) {
            ...
            var place = ($(window).width() > 992 ? $(this).data('center') :
                $(this).data('center-mobile')).split(',', 2);

            myPlacemark1 = new ymaps.Placemark([+place[0], +place[1]], {
    ...
    if ($(window).width() >= 992) {
        ...
        $('.contats__departament ').click(function(event) {
            ...
            var center1 = $(window).width() > 992 ? $(this).data('center') :
                $('this').data('center-mobile');
            center1 = center1.split(',', 2);

            myMap.setCenter([+center1[0], +center1[1]]);
            $('.contacts__info-address').html($(this).children('.contats__departament-address').text());
